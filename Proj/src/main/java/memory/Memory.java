 ...
 */
public class Memory {
    public static boolean SEGMENT = true;//是否分段
    public static boolean PAGE = true;//分段后是否分页
    ...
    private final PageItem[] pageTbl = new PageItem[Disk.DISK_SIZE_B / Memory.PAGE_SIZE_B]; // 页表（虚拟页）
    private boolean[] pageValid = new boolean[MEM_SIZE_B / PAGE_SIZE_B];//显示物理页哪些是有效的
    ...
    public void write(String pAddr, int len, byte[] data) {
        ...
        // 通知Cache缓存失效
        if (Cache.isAvailable) Cache.getCache().invalid(pAddr, len);
        // 更新数据
    ...
    public void seg_load(int segIndex) {
        ...
        //根据实验手册3.2.3，为简单起见，本实验只有一个段，base全0、limit全1、粒度与PAGE一致
        sd.base = "00000000000000000000000000000000".toCharArray();
        sd.limit = "11111111111111111111".toCharArray();
        sd.validBit = true;
        sd.granularity = PAGE;
        if (!PAGE) {//这里只处理分段，段页在下面page_load处理
            //只有一个段，内存地址对应磁盘地址，且从磁盘读取后加载到内存的开头位置。
    ...
    public void page_load(int vPageNo) {
        ...
        for (int i = 0; i < pageValid.length; i++) {
            if (!pageValid[i]) {//TODO:如果物理页已经满了、找不到空位怎么办呢
                pageNo = i;
                pageValid[i] = true;
                break;
            }
        ...
        write(frameAddr, PAGE_SIZE_B, Disk.getDisk().read(start, PAGE_SIZE_B));
    ...
    public void clear() {
        ...
        for (PageItem pItem : pageTbl) {
            if (pItem != null) pItem.isInMem = false;
        }
