...
public class TLB {
    ...
    private static final TLB tlbInstance = new TLB();// 单例模式
    private TLB() { }
    ...
    public void clear() {
        for (TLBLine tlbLine : TLB) {
            if (tlbLine != null) tlbLine.valid = false;
        }
    }
    ...
    private TLBLine getTLBLine(int index) {
        if (TLB[index] == null) TLB[index] = new TLBLine();
        return TLB[index];
    }
    ...
    private long getTimeStamp(int row) {
        ...
        if (tlbLine.valid) return tlbLine.timeStamp;
        return -1;
    }
    ...
    public boolean checkStatus(int[] lineNOs, boolean[] validations, int[] vPageNos, char[][] pageFrames) {
        ...
        for (int i = 0; i < lineNOs.length; i++) {
            ...
            if (line.valid != validations[i]) return false;
            if (line.vPageNo != vPageNos[i]) return false;
            if (!Arrays.equals(line.pageFrame, pageFrames[i])) return false;
        }
        return true;
    }
}
